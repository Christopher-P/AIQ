...
def build_task_specific_modules(task, model, d_sent, d_emb, vocab, embedder, args):
    ...
    ## NEW CHRIS CODE
    elif isinstance(task, CustomChrisTask):
        module = build_multiple_choice_module(
            task, d_sent, project_before_pooling=model.project_before_pooling, params=task_params
        )
        setattr(model, "%s_mdl" % str(task.name + "boolq"), module)
        module = build_pair_sentence_module(task, d_sent, model=model, params=task_params)
        setattr(model, "%s_mdl" % task.name, module)
...
class MultiTaskModel(nn.Module):
    ...
    def forward(self, task, batch, predict=False):
        ...
        ## CHRIS NEW CODE spot 2
        elif isinstance(task, CustomChrisTask):
            name_map = {"COPATask": "self._mc_forward", 
                        "BooleanQuestionTask": "self._pair_sentence_forward"}
            #model_map = {"COPATask": 0, "BooleanQuestionTask": 1}
            name = CustomChrisTask._custom_chris_forward(self, batch, task, predict)
            out_func = eval(name_map[name])
            out = out_func(batch, task, predict)
    ...
    def _get_classifier(self, task, n2=''):
        ...
        return getattr(self, "%s_mdl" % str(use_clf + n2))
    ...
    def _pair_sentence_forward(self, batch, task, predict):
        out = {}
        classifier = self._get_classifier(task, 'boolq')
