...
class ReallyNominalName(CustomChrisTask):
    ...
    def __init__(self, path, max_seq_len, name, **kw):
        ...
        self.proportion  = float(conf['proportion'])
        self.test_name_1 = str(conf['test_name_1'])
        self.test_name_2 = str(conf['test_name_2'])

        # Save it to the results (so we can keep track of it)
        with open('../jiant_data/bert-base-cased/results.tsv', 'a', newline='') as csvfile:
            spamwriter = csv.writer(csvfile, delimiter='\t',
                                    quotechar='|', quoting=csv.QUOTE_MINIMAL)
            spamwriter.writerow([self.proportion, self.test_name_1, self.test_name_2])

        # Get name of two tasks
        str_one = name_map[self.test_name_1] + "('{0}', {1}, '{2}')"
        str_two = name_map[self.test_name_2] + "('{0}', {1}, '{2}')"

        # Create instances of both tests
        # Expects /home/chris/aiq/dev/jiant/data/CoLA/ 256 cola
        test_A = str_one.format(data_path + data_map[self.test_name_1], 
                                      max_seq_len, 
                                      self.test_name_1)
        test_B = str_two.format(data_path + data_map[self.test_name_2], 
                                      max_seq_len, 
                                      self.test_name_2)
        self.A = eval(test_A)
        self.B = eval(test_B)
    ...
    def load_data(self):
        ...
        A_tr_data  = self.A.train_data_text
        A_te_data  = self.A.test_data_text
        A_val_data = self.A.train_data_text

        self.B.load_data()
        
        B_tr_data  = self.B.train_data_text
